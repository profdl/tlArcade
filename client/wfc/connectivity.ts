/**
 * WFC CONNECTIVITY — find the playable region and place food in it.
 * =================================================================
 ...
 * pockets with sealed walls between them. The fish navigate the tank as a graph of
 * rooms joined by doorways (registerSwimming's buildRoomGraph + roomPath), so a
 * food pellet stranded in an unreachable pocket is a pellet no fish can ever eat.
 *
 ...
 *   3. place green FOOD across that single region.
 *
 * PURE (no editor/DOM) → tested under `yarn test`. generateTank.ts consumes the
 * result and is the only impure step.
 */
...
/**
 * Choose up to `count` FOOD cells from the playable region, deterministically from
 * the grid's own ordering plus a stride so the pellets spread out rather than
 * clumping in one corner. We never place food in an unreachable pocket (we only draw
 * from `region`), so every pellet is reachable. Returns the chosen cells.
 *
 * Deterministic and pure: same grid + count → same pellets (so a seed reproduces the
 * whole tank, food included). `region` is expected to be largestComponent(grid).
 */
export function chooseFood(region: Cell[], count: number): Cell[] {
	if (region.length === 0 || count <= 0) return []
	const n = Math.min(count, region.length)
	// Even stride across the region list so pellets are spread, not adjacent.
	const stride = Math.max(1, Math.floor(region.length / n))
	const chosen: Cell[] = []
	for (let i = 0; i < n; i++) chosen.push(region[(i * stride) % region.length])
	return chosen
}
