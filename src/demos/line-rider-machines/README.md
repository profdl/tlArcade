# tldraw Line Rider: Machines

> Part of the [tlArcade](../../../README.md) prototyping platform — mounted
> at `/demos/line-rider-machines`. No `package.json`/build of its own; see
> "Run it" below. Forked from `line-rider-classic`.
...
- **Portals** *(done, v1)*: draw an arrow bound at both ends to geo shapes and
  ...
  the shapes' rotation difference. See [portals.ts](game/portals.ts).
...
- **Multipliers** *(done)*: wire a SECOND arrow out of an existing portal's
  ...
  [portals.ts](game/portals.ts)'s `splitBody` and
  [runController.ts](game/runController.ts).
