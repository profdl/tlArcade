/**
 * rig-play — RigTool: draw bones as a native tldraw tool.
 *
 * A custom StateNode (tldraw's "Custom tool" example) so bone-drawing OWNS the pointer
 * while active. Interaction: pointer DOWN sets a bone's PIVOT (snapped to a nearby tip →
 * child); drag; pointer UP sets the TIP and commits the bone. All draft coords are
 * ENTITY-LOCAL (page minus the target's bounds top-left). Copied from the Engine demo.
 */
import { StateNode, Vec } from 'tldraw'
import { snapParentForStart, snappedStart, type Vec2 } from '../rig/authoring'
import { boneCounterAtom, draftRigAtom, dragBoneAtom, rigTargetAtom } from './rigState'

/** Tip-snap radius in PAGE px. */
...
export class RigTool extends StateNode {
  // A simple id (no dot) — setCurrentTool treats a dotted id as a state PATH, so a root
  // tool must have an unqualified id.
