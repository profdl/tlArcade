...
import { aabbFullyInsideUnion, aabbOverlaps, clearOfObstacles, findClearPoint, resolveMove, type AABB } from '../collision'
...
describe('resolveMove', () => {
	...
	it('is blocked by a solid obstacle (a child room)', () => {
		const obstacle: AABB = { x: 60, y: 40, w: 30, h: 30 }
		// Moving right into the obstacle is rejected; the player stays put on x.
		const r = resolveMove(player(45, 50), 10, 0, [room], [obstacle])
		expect(r.x).toBe(45)
		// Moving left (away from it) is fine.
		const r2 = resolveMove(player(45, 50), -10, 0, [room], [obstacle])
		expect(r2.x).toBe(35)
	})
})

describe('clearOfObstacles', () => {
	it('is false when overlapping any obstacle, true otherwise', () => {
		const obs = [{ x: 60, y: 40, w: 30, h: 30 }]
		expect(clearOfObstacles(player(58, 50), obs)).toBe(false)
		expect(clearOfObstacles(player(10, 10), obs)).toBe(true)
	})
})

describe('findClearPoint', () => {
	it('returns the preferred point when it is already clear', () => {
		expect(findClearPoint(50, 50, 10, [room], [])).toEqual({ x: 50, y: 50 })
	})

	it('nudges off a solid child at the room centre to a clear, in-bounds spot', () => {
		// A child room covering the centre; the preferred point (centre) is inside it.
		const child: AABB = { x: 30, y: 30, w: 50, h: 50 }
		const p = findClearPoint(50, 50, 10, [room], [child])
		const box: AABB = { x: p.x - 5, y: p.y - 5, w: 10, h: 10 }
		expect(aabbFullyInsideUnion(box, [room])).toBe(true)
		expect(clearOfObstacles(box, [child])).toBe(true)
	})
})
