...
import type { StyleName } from './game/styles'
...
export default function App() {
	// The live game handle (set on mount) lets the style buttons rebuild the world in place.
	const gameRef = useRef<GameHandle | null>(null)
	const [style, setStyle] = useState<StyleName>(DEFAULT_CONFIG.style)

	const handleMount = useCallback((editor: Editor) => {
		editor.setCurrentTool('select')
		editor.updateInstanceState({ isReadonly: false })
		// Widen the zoom range past tldraw's native 800% max so the deepest scale (framed at
		// ~900%, see MAX_DEPTH in constants.ts) can be reached by zoomToBounds.
	...
	// Rebuild the whole nested world under the picked style (same seed → comparable worlds).
	const onPick = useCallback((next: StyleName) => {
		setStyle(next)
		gameRef.current?.regenerate({ style: next })
	}, [])
	...
	// PlayerSnail paints the snail over the (invisible) player; WorldControls is the style picker.
	const components: TLComponents = useMemo(
		() => ({
			...
			InFrontOfTheCanvas: () => (
				<>
					<PlayerSnail />
					<WorldControls style={style} onPick={onPick} />
				</>
			),
		}),
		[style, onPick]
