...
const ANCHORS = {
  ...
  legR: { joint: { x: 0.6, y: 0.77 }, tip: { x: 0.544, y: 0.986 } },
  legL: { joint: { x: 0.36, y: 0.78 }, tip: { x: 0.31, y: 1.0 } },
...
export function builderRig(figW: number, figH: number, ids: BuilderLimbIds): Rig {
  ...
  const bones: Bone[] = [
    ...
    limbBone('legL', ANCHORS.legL, figW, figH, neckX, neckY),
    limbBone('legR', ANCHORS.legR, figW, figH, neckX, neckY),
  ]

  // One rigid slot per driven leaf. The head bone drives the head ellipse AND the
  // smile + both eyes, so the face nods/leans as one unit. Draw order preserves the
  // capture order (torso behind, face on top) — the evaluator doesn't reorder.
...
/** The limb bones the walk swing drives (kept in sync with builderRig). */
