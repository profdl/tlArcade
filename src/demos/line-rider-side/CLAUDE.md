...
> mounted at `/demos/line-rider-side`. It has no `package.json`/build of its
> own — everything below (`npm run dev`/`build`/`test`/`lint`) runs from the
> **repo root**. It diverged from the same base as `line-rider-classic`
> rather than forking it directly — it does NOT have portals/flags.
