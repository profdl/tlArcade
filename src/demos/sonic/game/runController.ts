...
export interface SubstepResult {
	...
	scored: boolean
	/** True on the substep the body first reaches the goal (the win edge). Fires
	 * once per run — the controller latches `won` so later substeps don't re-fire. */
	won: boolean
}
...
export class RunController {
	...
	/** Whether the goal has been reached this run (latched). */
	get hasWon(): boolean {
		return this.won
	}
	...
	stepFixed(dt: number, contacts: ContactEvent[]): SubstepResult {
		...
		const center = bodyCenter(this.body)
		let scored = false
		if (this.checkpoints.length > 0) {
			const hits = collectCheckpointHits(center, this.checkpoints, this.collected)
			if (hits.length > 0) scored = true
		}
		// Win on the first substep the body center reaches the goal. Latched so it
		// fires exactly once — the loop reacts to the returned `won` edge (stops the
		// run, shows the banner). Testing per substep (like scoring) means a fast
		// sled can't tunnel past the goal between rendered frames.
		let won = false
		if (!this.won && reachedGoal(center, this.goalBox)) {
			this.won = true
			won = true
		}
		return { contacts, scored, won }
	}
