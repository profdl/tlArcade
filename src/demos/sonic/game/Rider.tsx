...
export function Rider() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (reseated) {
        ...
        wonAtom.set(false); // clear the win banner when re-seating (reset / start move)
      }
      if (runStarted) {
        scoreAtom.set({ collected: 0, total: run.currentCheckpoints.length });
        statsAtom.set({ distance: 0, speed: 0 });
        wonAtom.set(false); // a fresh run hasn't been won yet
        audio.beginRun();
        last = now;
        acc = 0;
      ...
      if (isPlaying) {
        ...
        // Win edge: the character reached the goal this frame. Raise the banner and
        // stop the run (leave the body where it finished, so the win pose reads).
        // A subsequent Reset / start-move clears wonAtom and re-arms the run.
        if (won) {
          wonAtom.set(true);
  ...
  // screen space each frame. Static appearance lives in App.css (.sonic-snail / the
  // start marker .sonic-start-*).
  return (
    <svg className="sonic-sled-svg" aria-hidden="true">
      ...
      <line ref={groundRef} className="sonic-ground" opacity="0" />
      ...
      <g ref={startRef} className="sonic-start-marker" opacity="0">
        <circle className="sonic-start-ring" r={START_RING_R} cy={START_RING_CY} />
      </g>
      ...
      <g ref={snailRef} className="sonic-snail">
        <g ref={snailFullRef}>
          <SnailArt />
        </g>
