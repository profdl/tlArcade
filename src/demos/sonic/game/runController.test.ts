...
function stubTrack(
	...
	setGoal: (g: Checkpoint | null) => void
} {
	let segs = segments
	let cps = checkpoints
	let goal: Checkpoint | null = null
	return {
		segments: () => segs,
		checkpoints: () => cps,
		goal: () => goal,
...
describe('RunController: stepping + scoring', () => {
	...
	it('wins the run once when the body center reaches the goal, then latches', () => {
		// Goal box right at the spawn so the body center is inside from step one.
		const track = stubTrack([floor])
		track.setGoal(flagAt('goal', 0, 0))
		const c = new RunController(track, inputs({ start: { x: 0, y: 0 } }))
		c.sync(inputs({ playing: true }))
		const first = c.stepFixed(DT, [])
		expect(first.won).toBe(true)
		expect(c.hasWon).toBe(true)
		// Latched: a second substep still inside the goal does not re-fire the edge.
		const second = c.stepFixed(DT, [])
		expect(second.won).toBe(false)
		expect(c.hasWon).toBe(true)
	})

	it('does not win when there is no goal', () => {
		const c = new RunController(stubTrack([floor]), inputs())
		c.sync(inputs({ playing: true }))
		expect(c.stepFixed(DT, []).won).toBe(false)
		expect(c.hasWon).toBe(false)
	})

	it('re-arms the win on the next run (after a reset)', () => {
		const start = { x: 0, y: 0 }
		const track = stubTrack([floor])
		track.setGoal(flagAt('goal', 0, 0))
		const c = new RunController(track, inputs({ start }))
		c.sync(inputs({ start, playing: true }))
		expect(c.stepFixed(DT, []).won).toBe(true)
		c.sync(inputs({ start, playing: false, resetNonce: 1 })) // Reset
